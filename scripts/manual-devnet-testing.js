...
async function testSubscriptionFlows() {
  ...
  try {
    const freemiumResult = await subscriptionService.subscribe(
      TEST_CONFIG.userId,
      "freemium"
    );
    logTest(
  ...
  try {
    const basicResult = await subscriptionService.subscribe(
      TEST_CONFIG.userId,
      "basic"
    );
    logTest(
      "Basic Subscription",
      basicResult.success &&
        basicResult.creditsGranted === 20 &&
        basicResult.burnedAmount === 1.8,
  ...
  try {
    const proResult = await subscriptionService.subscribe(
      TEST_CONFIG.userId,
      "pro"
    );
    logTest(
  ...
  try {
    const enterpriseResult = await subscriptionService.subscribe(
      TEST_CONFIG.userId,
      "enterprise"
    );
    logTest(
  ...
  try {
    const creditResult = await subscriptionService.useCredits(
      TEST_CONFIG.userId,
      5
    );
    logTest(
  ...
  try {
    const featureAccess = await subscriptionService.hasFeatureAccess(
      TEST_CONFIG.userId,
      "Advanced Analytics"
    );
    logTest(
...
async function testBurningMechanisms() {
  ...
  try {
    const mintingBurn = await burningService.burnMintingFee(
      TEST_CONFIG.userId,
      TEST_CONFIG.testAmounts.minting,
      TEST_CONFIG.agentId
    );
    logTest(
  ...
  try {
    const subscriptionBurn = await burningService.burnSubscriptionFee(
      TEST_CONFIG.userId,
      TEST_CONFIG.testAmounts.subscription,
      "pro"
    );
    logTest(
      "Subscription Fee Burning (20%)",
      subscriptionBurn.success &&
        Math.abs(subscriptionBurn.burnedAmount - 5.8) < 0.01,
  ...
  try {
    const upgradeBurn = await burningService.burnUpgradeFee(
      TEST_CONFIG.userId,
      TEST_CONFIG.testAmounts.upgrade,
      TEST_CONFIG.agentId
    );
    logTest(
  ...
  try {
    const marketplaceBurn = await burningService.burnMarketplaceFee(
      TEST_CONFIG.userId,
      TEST_CONFIG.testAmounts.marketplace,
      TEST_CONFIG.agentId
    );
    logTest(
  ...
  try {
    const daoBurn = await burningService.burnDAOTreasury(
      TEST_CONFIG.testAmounts.dao,
      "proposal-123"
    );
    logTest(
...
async function testAgentOperations() {
  ...
  try {
    const evolutionResult = await agentService.evolveAgent(
      TEST_CONFIG.agentId,
      {
        userId: TEST_CONFIG.userId,
...
async function testEconomicIntegration() {
  ...
  try {
    ...
    // Use credits
    const creditUsed = await subscriptionService.useCredits(
      TEST_CONFIG.userId,
      10
    );
